...
tabla_correlaciones %>%
  knitr::kable(
    caption = "Correlación entre el ranking nacional IGVUST y otros índices comunales (Spearman)",
    digits = 2,
    format = "html"
  )
```
...
El ranking nacional IGVUST (`rank_nac`) asigna el valor 1 a la comuna más vulnerable del país. Por lo tanto, una correlación **negativa** con `rank_nac` indica que el índice comparado aumenta a medida que la vulnerabilidad IGVUST es mayor (comunas más vulnerables tienen valores más altos de ese índice), mientras que una correlación **positiva** indica lo contrario: el índice aumenta junto con la disminución de la vulnerabilidad (comunas menos vulnerables tienen valores más altos de ese índice).

Ejemplo: una correlación de Spearman = -0.41 con el índice de envejecimiento significa que las comunas más vulnerables (rank bajo) tienden a tener mayor envejecimiento poblacional.
:::
